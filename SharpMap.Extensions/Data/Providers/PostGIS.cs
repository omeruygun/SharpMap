    ...
    public class PostGIS : BaseProvider
    {
        ...
        public override Collection<IGeometry> GetGeometriesInView(Envelope bbox)
        {
            ...
            using (var conn = new NpgsqlConnection(ConnectionString))
            {
                ...
                string sqlString = "SELECT a.geom::bytea AS \"_smGeom_\"\n" +
                "  FROM ( SELECT (ST_Dump(p_geom)).geom AS geom\n" +
                "         FROM (SELECT \""+GeometryColumn+"\" AS p_geom from "+ QualifiedTable + ") AS b\n" +
                "        ) AS a where ";
                ...
                {
                    strSQL += "st_intersects(\"" + GeometryColumn + "\"," + GetBoxClause(bbox) + ") and ST_GeometryType(\"SP_GEOMETRY\")!='ST_GeometryCollection'";
                    sqlString += "st_intersects(a.geom,"+ GetBoxClause(bbox) + ")";
                }
                else
                {
